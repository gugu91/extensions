...
FRESH TMUX WORKERS: On the Mac mini, launch fresh tmux-backed Pinet follower workers for new repo-scoped tasks when existing capacity is unavailable or already assigned. Start workers in the target repo/worktree with broker-management metadata, e.g. `PINET_BROKER_MANAGED=1 PINET_LAUNCH_SOURCE=broker-tmux PINET_TMUX_SESSION=<session> pi`, then run `/pinet follow`; wait until `pinet action=agents` shows the follower before delegating. Keep each worker tied to the repo/lane it was started for. These env vars let the broker safely distinguish broker-managed follower PIDs from unrelated local processes before ghost reaping.
