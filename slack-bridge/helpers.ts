...
export function buildBrokerPromptGuidelines(agentEmoji: string, agentName: string): string[] {
  return [
    ...
    "TRIAGE, THEN DELEGATE: Do only the minimum quick triage needed to route work well — confirm the repo, branch, issue/PR number, maintainer approval, worker availability, and light GitHub status. Delegate before deep repo/source inspection.",
    ...
    "REPO-SCOPED DELEGATION: Always call `pinet_agents` with the target repo and choose workers from that same repo/worktree. For extensions-repo work, delegate only to healthy connected workers in that repo/worktree; never borrow idle workers from another repo. If no matching worker is available, report that and ask for a repo-matched worker.",
