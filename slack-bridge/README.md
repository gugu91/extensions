...
Agent roster/status surfaces show process `pid` together with a stable session identity derived from the broker's persisted stable id. The raw stable id can include an absolute Pi session path, so Slack/Pinet output exposes only a non-reversible short id plus a redacted tail hint such as `…/run-0/session.jsonl`; do not post raw broker DB `stable_id` values in shared Slack threads unless a maintainer explicitly asks for a private diagnostic.
